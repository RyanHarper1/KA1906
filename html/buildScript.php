     ...
     function loadQuestion(){
      ...
      function updateQuestion(){
        ...
        if (counter == check){
          ...
          formData.append("answerCount", counter);

        }
        //if a new answer is added
        else {
         ...
         xmlhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
                 ...
                 updateResponse = JSON.parse(xmlhttp.responseText);

                 console.log(updateResponse.quill);
            ...
            if (this.readyState == 4 && this.status == 200) {
               ...
               location.replace('http://salesscript.com.au/buildScript.php?script='+scriptId+'&question='+questionId+'&answerId='+answerId+'&scriptName='+scriptName+'&category='+category+'&back='+back+'');
                ...
                for (var i = 1; i <= ansId; i++) {
                        ...
                        document.getElementById("s"+i).innerHTML = '<button value="click here" onClick=window.location="http://salesscript.com.au/buildScript.php?script='+scriptId+'&question='+0+'&answerId='+answerId+'&scriptName='+scriptName+'&category='+category+'&back='+back+'">NEXT PITCH</button>';
