<?php
  ...
  $question   = $_GET["question"]; //store questionId
  $scriptName = $_GET["scriptName"]; //store scriptname
  $scriptId   = $_GET["script"]; //store script id
  $category   = $_GET["category"]; //store category
  $button     = 0;

  //Execute query to display script
  $results = mysqli_query($dbConnection, "SELECT * FROM answer WHERE nextQuestionId = '$question'");

  $result = mysqli_fetch_array($results); //store results of query
  $question = $result['questionId']; //store question id


  //check if first question of the script when viewing
  $question1 = $_GET["question"];
  $results1  = mysqli_query($dbConnection, "SELECT * FROM script WHERE firstQuestionId = '$question1'"); //check if the question being loaded is the first question of this script
  ...
  //do not display back button on first question of script
  if ($scriptId == 0) {
      $button = 1;
  }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
           ...
           if (<?php echo $button ?> == 0){
                //change back buttons depending on whether a completely new pitch or existing pitch
                if (questionId == 0){
          ...
          //edit button to allow users to start editing from pitch they are viewing
          ...
          if (questionId != 0){
            ...
            $(add_button).click(function(e){
                ...
                if(x < max_fields){
                 ...
                 alert('You Reached the limits'); //alert if 10 answer limit has been reached
               }
            });
      ...
      //display pitch and answers to user
     function loadQuestion(){
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                counter = loadResponse.i; //question counter
                counter = counter - 1; //minus one from counter to avoid out of bounds error
                pitch = loadResponse[0].texts; //load pitch
                quill.setText(pitch); //set quill to pitch
                scriptname = loadResponse.scriptName; //script name
                category = loadResponse.category; //category
                back = loadResponse.backId; //back id
                var ans = 0;

                //dynamically create amount of text boxes needed to display answers per pitch
                for (var i = 1; i <= counter; i++) {
                  //check whether an answer has a follow through pitch
                ...
                //if answer has a pitch to go to when clicked
                if (questionId != 0) {

                  //dynamically create text box and display answer
                ...
                //if a answer does not have a question to go to
		            } else {
                  //dynamically create text box and display answer
		                $(wrapper).append('<div class="col-sm-4"> <div class="panel panel-primary" style="box-shadow: 15px 15px black;"> <div class="panel-heading" >Answer <span id="s'+i+'"> </span></div><div class="panel-body">  <input type="text" style="width:100%; border: none;" id="a'+i+'" name="a'+i+'"></div></div> </div>'); //add input box
                    document.getElementById("a"+i).value= loadResponse[i].id;

                    //hide next pitch button
